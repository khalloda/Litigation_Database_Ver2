        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h3 class="card-title">{{ __('app.edit_contact') }}</h3>
                    ...
                    <form method="POST" action="{{ route('contacts.update', $contact) }}">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('contact_type') is-invalid @enderror" id="contact_type" name="contact_type" required>
                                        ...
                                        <option value="phone" {{ (old('contact_type', $contact->contact_type) == 'phone') ? 'selected' : '' }}>{{ __('app.phone') }}</option>
                                        <option value="mobile" {{ (old('contact_type', $contact->contact_type) == 'mobile') ? 'selected' : '' }}>{{ __('app.mobile') }}</option>
                                        <option value="email" {{ (old('contact_type', $contact->contact_type) == 'email') ? 'selected' : '' }}>{{ __('app.email') }}</option>
                                        <option value="fax" {{ (old('contact_type', $contact->contact_type) == 'fax') ? 'selected' : '' }}>{{ __('app.fax') }}</option>
                                        <option value="website" {{ (old('contact_type', $contact->contact_type) == 'website') ? 'selected' : '' }}>{{ __('app.website') }}</option>
                                        <option value="address" {{ (old('contact_type', $contact->contact_type) == 'address') ? 'selected' : '' }}>{{ __('app.address') }}</option>
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="fas fa-save"></i> {{ __('app.update_contact') }}
                            </button>
                        </div>
                    </form>
                </div>
