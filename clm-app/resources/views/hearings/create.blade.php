...
@section('title', __('app.create_hearing'))

@section('content')
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h4">{{ __('app.create_hearing') }}</h1>
        <a href="{{ route('hearings.index') }}" class="btn btn-outline-secondary">{{ __('app.cancel') }}</a>
    </div>
            ...
            <form action="{{ route('hearings.store') }}" method="POST">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control @error('court') is-invalid @enderror" id="court" name="court" value="{{ old('court') }}">
                        @error('court')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control @error('next_hearing') is-invalid @enderror" id="next_hearing" name="next_hearing" value="{{ old('next_hearing') }}">
                        @error('next_hearing')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="d-flex justify-content-end">
                    <a href="{{ route('hearings.index') }}" class="btn btn-secondary me-2">{{ __('app.cancel') }}</a>
