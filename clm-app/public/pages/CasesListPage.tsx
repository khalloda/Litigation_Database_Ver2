...
const CasesListPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Service functions now handle pagination extraction
        setCases(Array.isArray(casesData) ? casesData : []);
        setClients(Array.isArray(clientsData) ? clientsData : []);
        setOpponents(Array.isArray(opponentsData) ? opponentsData : []);
        setLawyers(Array.isArray(lawyersData) ? lawyersData : []);
        setCapacities(Array.isArray(capacitiesData) ? capacitiesData : []);
        setStatuses(Array.isArray(statusesData) ? statusesData : []);
        
        // Log for debugging
        console.log('Loaded data:', {
          cases: Array.isArray(casesData) ? casesData.length : 'not array',
          clients: Array.isArray(clientsData) ? clientsData.length : 'not array',
          lawyers: Array.isArray(lawyersData) ? lawyersData.length : 'not array',
        });
      } catch (err: any) {
  ...
  const filteredCases = useMemo(() => {
    ...
    // Debug: Log first case structure
    if (cases.length > 0) {
      console.log('First case structure:', cases[0]);
      console.log('Total cases to filter:', cases.length);
    }
    
    const filtered = cases.filter(c => {
      // Check if client exists
      if (!c.client) {
        console.warn('Case missing client:', c.id);
        return false;
      }
      
      // Check if opponents is an array
      ...
      if (filters.opponent && (!c.opponents || !c.opponents.some(o => o?.id === parseInt(filters.opponent, 10)))) return false;
      if (filters.opponentCapacity && (!c.opponents || !c.opponents.some(o => o?.capacity === filters.opponentCapacity))) return false;
  ...
  return (
    <div className="container mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">{t('cases_page.title')}</h1>
