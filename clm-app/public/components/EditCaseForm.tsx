...
import { fetchCase, updateCase } from '../services/cases';
...
const EditCaseForm: React.FC<EditCaseFormProps> = ({ caseId, onClose, onSave }) => {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                setLoading(true);
                const [caseData, clientsData, opponentsData, lawyersData, courtsData] = await Promise.all([
                    fetchCase(caseId),
                    fetchClients(),
                    fetchOpponents(),
                    fetchLawyers(),
                ...
                const caseRecord = caseData?.data ?? caseData;
                ...
                setFormData({
                    caseNameEn: caseRecord.case_name_en || caseRecord.matter_name_en || '',
                    caseNameAr: caseRecord.case_name_ar || caseRecord.matter_name_ar || '',
                    description: caseRecord.case_description || caseRecord.matter_description || '',
                    clientId: caseRecord.client_id ? String(caseRecord.client_id) : '',
                    opponentId: caseRecord.opponent_id ? String(caseRecord.opponent_id) : '',
    ...
    return (
        <Modal title={t('edit_case_form.title') || 'Edit Case'} onClose={onClose}>
            {loading ? (
