<!-- cb2fbaf6-6e3a-48f9-8330-79022f5f1db9 4d009fce-ee92-42e7-8048-23129bcfd845 -->
...
- [ ] Install react-router-dom and TypeScript types
- [ ] Refactor App.tsx to use React Router with all routes mapped
- [ ] Update Layout.tsx to use React Router hooks (useLocation, useNavigate, Outlet)
- [ ] Create services/api.ts with axios instance and interceptors
- [ ] Create all domain service files (cases, clients, hearings, documents, etc.)
- [ ] Create services/auth.ts with login/logout/fetchCurrentUser functions
- [ ] Create services/ai.ts to proxy Gemini calls through Laravel
- [ ] Update DashboardPage.tsx to fetch cases from API instead of mock data
- [ ] Update CaseDetailPage.tsx to use useParams and fetch case from API
- [ ] Update all list pages (Clients, Opponents, Lawyers, Courts, Hearings, Documents, Tasks) to use API
- [ ] Update all detail pages to use useParams and fetch from API
- [ ] Update form components (NewCaseForm, NewHearingForm, etc.) to submit via API
- [ ] Create ProtectedRoute component for authentication guard
- [ ] Wrap all protected routes in App.tsx with ProtectedRoute component
- [ ] Implement file upload in documents service and UpdateDocumentPage.tsx
- [ ] Replace direct Gemini API calls in CaseCard and DocumentAnalyzer with AI service
- [ ] Update vite.config.ts for Laravel deployment (remove Gemini API key)
- [ ] Install axios package if not already present
- [ ] Delete services/mockData.ts and services/database.ts after migration verified
- [ ] Remove @google/genai from package.json dependencies